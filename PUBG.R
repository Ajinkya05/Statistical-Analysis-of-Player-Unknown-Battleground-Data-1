...
#Spring 2018
#Final Project
#Player statistical analysis of PUBG game
#Group Members - Gaurav Kshirsagar, Shrawan Sapre, Ajinkya Ingle.
...
d <- read.csv('/Users/grv/Desktop/Spring 18/513/Project 513/PUBG-Game-statistical-analysis/PUBG_Player_Statistics.csv', sep=',', stringsAsFactors=F)
summary(d)
...
for(i in 1:length(performances_features)){
  ...
  perfPlots[[i]]<- perf_d %>% dplyr::select(.dots=x,cat) %>%
    ...
    scale_fill_manual(name="",values=c("Blue","Green","Orange")) + facet_grid(~cat)
}

do.call(grid.arrange, c(perfPlots, ncol=2))

#Grouping of Performance features
length_features<-length(support_features) #taking only 5 significant sttributes.
...
#Creating samples with 7000 entries to reduce computing overhead.
train_solo_sample<-sample_n(train_solo, 7e3)
train_duo_sample<-sample_n(train_duo, 7e3)
train_squad_sample<-sample_n(train_squad, 7e3)
...
#Classifiers Training using 'train' function for performance tuning
classifiers<-c('knn','nb','rf','svmLinear','nnet')
trControl <- trainControl(method = "cv",number = 5) #using cross validation 
timing<-c()
res<-list()
cnt<-0
...
test_solo_sample<-sample_n(test_solo,7e3)
test_duo_sample<-sample_n(test_duo, 7e3)
test_squad_sample<-sample_n(test_squad, 7e3)
...
#Plotting optimal number of clusters for KNN
plot(res[[1]])

#Plotting most important features from rf
plot(varImp(object = res[[3]]))
...
mm <- makeDendogram(all_train,'All categories')
mm
